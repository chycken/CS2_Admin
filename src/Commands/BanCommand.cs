...
public class BanCommand : BanCommandBase
{
    public BanCommand(
        ISwiftlyCore core,
    ...
    public override async void Execute(ICommandContext context)
    {
        ...
        try
        {
            await HandleOnlineBan(context, false);
        }
        catch (Exception ex)
        {
    ...
    private async Task HandleOnlineBan(ICommandContext context, bool ipMode)
    {
        ...
        if (targetSnapshot != null)
        {
            ...
            if (!await PlayerUtils.CanAdminTargetAsync(Core, _adminDbManager, context, targetSnapshot.SteamId))
            {
                return;
            }
            ...
            var steamAlreadyBanned = false;
            var ipAlreadyBanned = false;
            ...
            if (shouldBanSteam)
            {
                ...
                {
                    _ = _banManager.AddBanAsync(targetSnapshot.SteamId, targetSnapshot.Name, duration, reason, isGlobal, targetSnapshot.IpAddress);
                }
                else
                {
                    steamAlreadyBanned = true;
                }
            }
            ...
            if (shouldBanIp)
            {
                ...
                {
                    ipAlreadyBanned = true;
                }
            }

            var steamBlocked = shouldBanSteam && steamAlreadyBanned;
            var ipBlocked = shouldBanIp && ipAlreadyBanned;
            if (steamBlocked || ipBlocked)
            {
                await OnMainThreadAsync(() => Reply(context, "player_already_banned", targetSnapshot.Name));
                return;
            }
        ...
        if (!ipMode && PlayerUtils.TryParseSteamId(targetArg, out var offlineSteamId))
        {
            ...
            if (shouldBanIp)
            {
                ...
                {
                    if (await AddOfflineIpBanAsync(context, knownIp, duration, reason, adminName, adminSteamId, isGlobal, notifyResult: false))
                    {
                        appliedCount++;
                    }
                }
